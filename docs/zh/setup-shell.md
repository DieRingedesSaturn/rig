...
| 路径 | 行为 |
|------|------|
| `~/.zshrc` | 默认只读。缺失的插件/Starship 初始化行仅在显式 `y/N` 确认后追加，且先自动备份。 |
| `~/.config/starship.toml` | 缺失时创建。已存在的配置会展示 diff 并提供 保留/覆盖/追加 菜单，覆盖/追加前自动备份。 |
| 默认登录 Shell | 只报告；仅在显式 `y/N` 确认后才通过 `chsh` 修改。 |
| 软件包 | 只通过发行版包管理器安装。唯一例外是下面的 Starship 回退方案。 |

没有可用 `/dev/tty`（非交互运行）时什么都不改——任何"必须改你文件才能完成"的事，都会在运行结束时以清单形式打印出来，由你自己决定是否执行。这样组件与用 dotfiles 仓库跟踪 `~/.zshrc` 和 `~/.config/starship.toml` 的做法完全兼容——全部拒绝后那些仓库仍然是干净的。
...
| `~/.zshrc` | 仅在显式 `y/N` 确认 + 备份后追加 |
| `/etc/passwd` | 登录 Shell 字段仅经 `y/N` 确认后由 `chsh` 修改 |

## 重复运行行为

完全幂等。已安装的包会跳过，已存在的 `starship.toml` 除非你显式选择否则不动，没有任何写入不需要确认——全部拒绝后磁盘不会有任何变化。
