...
这个组件在设计上就是非破坏性的。它绝不覆盖不是它自己创建的文件：

| 路径 | 行为 |
|------|------|
| `~/.zshrc` | **永远只读。** 不创建、不追加、不备份、不还原。 |
| `~/.config/starship.toml` | **仅在缺失时创建。** 已存在的配置绝不覆盖——连应用预设都不会做。 |
...
| 4/5 | **仅在缺失时**创建 `~/.config/starship.toml` |
...
| 文件 | 操作 |
|------|------|
| `~/.config/starship.toml` | 仅在缺失时创建；默认提示符显示路径、git 状态、语言运行时与时间，SSH 会话中额外显示红色 `ssh:<主机名>` 前缀 |
| `~/.zshrc` | **绝不触碰** |
| `/etc/passwd` | **绝不触碰** |
