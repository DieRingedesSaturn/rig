...
| `~/.zshrc`, `~/.bashrc` | Appends `export PATH="$HOME/.local/bin:$PATH"` only after an explicit `y/N` confirmation, with a timestamped backup first; without a TTY the line is only printed |
